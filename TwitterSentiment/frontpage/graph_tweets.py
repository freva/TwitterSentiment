...
def graphHashtags(hashtags, startTime, endTime):
    ...
    yAxis = [{"name": "Polarity", "type": "spline", "yAxis": 1, "data": [sum(pol)/len(pol) if len(pol)>0 else 0 for pol in polar]},
           {"name": "Frequency", "type": "column", "data": [len(pol) for pol in polar]}]
